...
class Zombie:
    ...
    def __init__(self, x=None, y=None):
        ...
        self.patrol_locations = [(43, 274), (1184, 274), (1050,  494), (575, 804), (235, 991), (575, 804), (1050, 494), (1118, 274)]
        self.loc_no = 0
    ...
    def is_boy_near_and_stronger(self, distance):
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y,
                                   distance) and play_mode.boy.ball_count > self.ball_count:
            return BehaviorTree.SUCCESS
        return BehaviorTree.FAIL

    def is_boy_near_and_weaker(self, distance):
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y,
                                   distance) and play_mode.boy.ball_count <= self.ball_count:
            return BehaviorTree.SUCCESS
        return BehaviorTree.FAIL
    ...
    def build_behavior_tree(self):
        ...
        a6 = Action('순찰 이동', self.move_to)
        patrol = Sequence('순찰', a5, a6)

        boy_stronger_condition = Condition('소년이 근처에 있고 강한가?', self.is_boy_near_and_stronger, 7)
        flee_action = Action('소년에게서 도망', self.flee_from_boy)
        flee_from_boy = Sequence('소년에게서 도망', boy_stronger_condition, flee_action)

        boy_weaker_condition = Condition('소년이 근처에 있고 약한가?', self.is_boy_near_and_weaker, 7)
        chase_action = Action('소년 추적', self.move_to_boy)
        chase_boy = Sequence('소년 추적', boy_weaker_condition, chase_action)

        chase_or_flee = Selector('추적 또는 도망', flee_from_boy, chase_boy)

        # 최종 행동 트리: 도망 > 추적 > 순찰
        root = Selector('좀비 행동 선택', chase_or_flee, patrol)
